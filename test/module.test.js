  ...
  throwRegisteredErrors(methodName) {
    const throwableDefinition = this.exceptionsToThrow.find(
      descriptor =>
        ...
        descriptor.attemptCounts.includes(this.attempts[methodName])
    );
    ...
    return {
      promise: () => {
        try {
          ...
          if (
            params.QueryString &&
            params.QueryString.toLowerCase().startsWith("select")
          ) {
  ...
  describe("Testing query method...", () => {
    it("Non DML query example", async function() {
      ...
      const result = await athenaExpress.query(
        "SHOW TABLES 'table_name_pattern'"
      );
      ...
      athenaExpress
        ...
        .catch(err => {
          try {
            ...
            expect(
              athenaExpress.config.athena.attempts[THROW_IN_GETQUERYEXECUTIOIN]
            ).to.be.equals(0);
            done();
          } catch (err) {
            done(err);
    ...
    it("(startQueryExecution) No Retry on non common AWS error", function(done) {
      ...
      athenaExpress.config.athena.throwErrorWhen(
        THROW_IN_STARRTQUERYEXECUTION,
        err,
        [1]
      );
      ...
      athenaExpress
        .query(
          "SELECT yearmonth_field FROM table_name order by yearmonth_field"
        )
        .then(() => done())
        .catch(err => {
          try {
    ...
    it("(startQueryExecution) Retry twice on common error", async function() {
      ...
      expect(
        athenaExpress.config.athena.attempts[THROW_IN_STARRTQUERYEXECUTION]
      ).to.be.equals(2);
    ...
    //TODO: add tests around retry and commonErrorRetry functions/settings
  ...
  describe("Testing configurable behaviors...", () => {
    it("Returns stats when getStats is true, cost per MB", async function() {
      ...
      expect(result).to.eql({
        ...
        S3Location:
          "s3://bucket_name/results/sql-log/s3_athena_utility_response.csv"
      });
    });
