...
module.exports = class AthenaExpress {
	 ...
	 *
	 * Examples:
	 * await query('SELECT * FROM movies WHERE movie_title = ?', ['Spider-Man']);
	 * await query({ sql: 'SELECT * FROM movies WHERE movie_title = ?', queryParams: ['Spider-Man']});
	 *
	...
	async query(query, queryParams) {
		...
		try {
			const queryExecutionId = await startQueryExecution(query, queryParams, config);
...
function startQueryExecution(query, queryParams, config) {
	const QueryString = query.sql ?
		SqlString.format(query.sql, query.queryParams || [])
		: SqlString.format(query, queryParams || []);
